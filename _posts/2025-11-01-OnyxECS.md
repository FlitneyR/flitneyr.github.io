...
layout: post
excerpt: Onyx employs a query driven ECS. This pattern is not all that new, with its roots in MVC software architecture. In this article I'll try and explain a bit about why I chose an ECS for my engine; how it is used in Onyx; and how I have gone about implementing it.
---

{% include init-mermaid.html %}
...
[Onyx]({{ site.baseurl }}{% link _projects/Onyx.md %}) employs a query driven ECS. This pattern is not all that new, with its roots
in MVC software architecture. Though not traditionally used in games, it has gained traction as of late, as an alternative to the
Actor / GameObject architecture from Unreal Engine / Unity respectively. In this article I'll try and explain a bit about why I
chose an ECS for my engine; how it is used in Onyx; and how I have gone about implementing it.
...
- [Implementation](#implementation)
    ...
    - [The World's Data Layout](#the-worlds-data-layout)
    - [Queries](#queries)
    - [Systems](#systems)
    - [The Command Buffer](#the-command-buffer)
- [Improvements](#improvements)
    - [System Scheduling](#system-scheduling)
    - [Component Paging](#component-paging)
    - [Messaging](#messaging)
...
Typically, gameplay logic is implemented using patterns that more closely couple behaviour with types.
Think, for example, of Actors in UE, or MonoBehaviours in Unity, which - though they do allow for separate concerns
to be divided into separate components - still combine data and behaviour into the same class.

These architectures also imply that gameplay logic applies to single entities at a time. But only a minority of
game objects will be singletons. Why optimise a system for an edge case? It is always possible - and there are many
cases where it is useful - to iterate over a large set of similar game objects and apply logic to them. One way to
circumvent this restriction in other engines is by creating a "Manager" Actor, which tracks some subset of your Actors,
to apply some gameplay logic to the group as a whole. A bit convoluted when you could just run a function over that set of entities.
...
So ECS provides a nice separation of data and logic, it's easy to compose new behaviour by combining components,
and as we'll explore later, by well-defining the interface of your systems, it opens the door to fearless concurrency.
...
First we want to define the context of our systems. These will be structs passed to our systems by reference or const reference.
They provide a way to pass data into/out of the ECS, as well as between systems.
...
<div class="note"> <p> <b>Note:</b>
A proper implementation would also need to define a <code class="language-plaintext highlighter-rouge">COMPONENT_REFLECTOR</code>
for each of these so that the component can be added in the editor, serialised, and deserialised. But that is outside the scope
of this article, so it has been omitted. As have most of the bodies of these structs, as the specifics of their implementation
are also unimportant for this article. </p> </div>
...
In Onyx, a system is defined by a function which takes one `Context< ... >` argument, followed by any number of
`const Query< ... >&` arguments. These define what context, and which sets of entities, the system will interact with.

The types within `Query` define which entities are a valid match for the query, and how we can view/modify the
resulting entities that the query finds.

| Component Query | Resulting variable | Description                                                                                        |
| --------------- | ------------------ | -------------------------------------------------------------------------------------------------- |
...
Each `Query< ... >` contains a set of `Query< ... >::Result`s. We can ask for specific entities from this set, using
`Query< ... >::Get( EntityID id )`, iterate through it with `Query< ... >::operator []( u32 index )`, or iterate through
it with `Query< ... >::begin/end()`.

These components can be accessed in one of two ways. Via `query_result.Get< T >()`, or all components can be retrieved
at once with the pattern:<br>`auto [ entity_id, component_1, component_2, ... ] = query_result.Break()`.
...
We have to put our various update functions in a `pre_update_set`, `update_set`, and `post_update_set`, so that they execute in the correct
order, and without writing to memory that another system is reading. Ideally we would have a [better solution to scheduling systems](#system-scheduling).
...
It is worth noting that any operation that adds or removes a components potentially invalidates some queries.
An entity that matched the query before may no longer match the query, or an entity that didn't match before
might match the query afterwards. Also, and this delves slightly into implementation, if adding a component
changes how the other components are stored in memory, then all the pointers we have cached in our query will now be invalid.

So if we want to be able to add/remove components within systems, we will need a way to record a buffer of these commands
to execute when no systems are running, and then recompute our queries.

For this we can access a Command Buffer:
...
const extern f32 s_healthFlashingThreshold;
const extern f32 s_healthFlashingRate;

void StartLowHealthFlashing(
    onyx::ecs::Context<
    ...
    const onyx::ecs::Query<
        ...
        onyx::ecs::ReadOptional< Flashing >
    >& entities
) {
    auto [ cmd ] = ctx.Break();
    ...
    {
        aut [ id, health, flashing ] = entity.Break();

        if ( flashing || health.amount > s_healthFlashingThreshold )
            cmd.AddComponent( id, Flashing( s_healthFlashingRate ) );
    }
}
{% endhighlight %}
...
Below is a _slightly simplified_ diagram of the class structure of Onyx's ECS.

{% include note.html content="Not all class internals are listed, and anywhere were we abstract a templated class I have
just listed here without the template. In reality, these are stored as unique or shared pointers to an interface extended
by the concrete template classes. But listing that out would overcomplicate this diagram, and render it entirely illegible.
Types also should not be taken literally, something labelled 'set' for example might in reality be implemented as a vector,
but conceptually used as a set." %}

<pre class="mermaid">
classDiagram
    class World {
        -Map~TypeHash, ComponentTable~ componentTables
        +QueryManager queryManager
        +AddComponent~Component~(EntityID, Component)
        +RemoveComponent~Component~(EntityID)
        +GetComponent~Component~(EntityID) Component
    }

    class ComponentIterator~Component~ {
        -u32 currentEntityComponentPair
        +GetEntity() EntityID
    ...
    class ComponentTable~Component~ {
        ...
        +GetIterator() ComponentIterator
        +AddComponent(EntityID, Component)
        +RemoveComponent(EntityID)
        +GetComponent(EntityID) Component?
    }
    class EntityIterator {
        -Map~TypeHash,ComponentIterator~ componentIterators
        +Get~Component~() Component?
        +GetEntity() EntityID
        +Increment()
    }

    World --* ComponentTable : composed of
    ComponentTable --> ComponentIterator : generates
    World --> EntityIterator : generates
    EntityIterator --* ComponentIterator : composed of

    class QueryManager {
        -Set~WeakPtr~Query~~ queries
        +Get~Query~() SharedPtr~Query~
    }
    class QuerySet {
        -Set~WeakPtr~Query~~ queries
        +Update()
    ...
    class Query~Components ...~ {
        -Vector~Result~Components ...~~ results
        +Get(EntityID) Result~Components ...~
    }
    class Result~Components ...~ {
        -EntityID entity
        -Tuple~Components ...~ components
        +GetEntity() EntityID
        +Get~Component~() Component
        +Break() Tuple~Components ...~
    }

    EntityIterator --> Query : used to generate

    QueryManager --* Query : generates
    World --* QueryManager : contains
    Query --* Result : composed of
    QuerySet --> Query : updates

    class SystemSet~Context~ {
        -Set~System~Context~~ systems
        +AddSystem~Function~( Function )
        +Run( Context )
    }
    class System~Context, Function~ {
        -Set~SharedPtr~Query~~ queries
        -Function function
        +Run( Context )
    }

    System --> QueryManager : gets queries from
    System --> QuerySet : registers queries with
    System --> Result : iterates over
    SystemSet --* System : composed of
    ...
    System --> CommandBuffer : sends commands to
</pre>

{% include note.html content="I've omitted a link from CommandBuffer to World labelled 'updates' as it was messing up the layout in a
way I couldn't resolve" %}

### The World's Data Layout
...
struct Entity
{
    ...
    Component* Get() const
    {
        if ( auto iter = components.find( Component::s_typeHash ); iter != components.end() )
            return static_cast< Component* >( iter->second.get() );
...
{% include note.html content="Some code has been omitted or simplified for brevity:
virtual destructors, public/private access specifiers, some standard library shenanigans, other code not directly relevant to this article" %}
...
Now we will define `IComponentTable`, which is essentially a wrapper for an associative array from `EntityID` to a certain
...
struct EntityIterator
{
    ...
    Component* Get() const
    {
        ...
        ComponentTable< Component >::Iterator iter = *static_cast< ComponentTable< Component >::Iterator* >( generic_iter->get() );
...
struct Query : IQuery
{
    ...
    const Result& nth( u32 index ) const { return m_results[ index ]; }
...
This approach has some benefits, and some drawbacks. Most of which I think could be alleviated by [paging components](#component-paging).
...
First there's a little bit more glue we need, a Context interface, and a System interface.
`Context< ... >` will allow us to request certain context arguments from within our systems.
This allows systems to be aware of / modify state outside of the ECS, without making that state global.
...
And our system interface will allow us to group systems together that can be run with the same context:

{% include note.html content="The IContext here doesn't refer to a specific Context interface class, just a context class that
can be, but is not necessarily, the same as the actual Context of the system. Context's constructor will resolve this so long as the system's
context is a subset of IContext" %}
...
struct System< IContext, void( Context, const Queries& ... ) > : ISystem< IContext >
{
    ...
    // get the queries in the constructor, before type erasure
...
And now we can finally create a system set. This will hold our systems, and a reference to a `QuerySet` to fetch our queries from.
The specific queries are fetched in `AddSystem`, before we erase the underlying type behind a `UniquePtr` to the underlying `ISystem` base class.
...
We also must implement a command buffer, but this just boils down to a queue of `UniquePtr< IComand >`s and some
wrapper functions to add them. I will not go in depth on the implementation here. A very simple implementation might
look something like:
...
struct AddComponent : ICommand
{
    EntityID entity;
    Component component;

    AddComponent( EntityID entity, Component component )
        : entity( entity )
        , component( component )
    {}
...
template< typename Component >
struct RemoveComponent : ICommand
{
    EntityID entity;
...
{
    Queue< UniquePtr< ICommand > > commands;

    template< typename Command, typename ... Args >
    void AddCommand( Args ... args )
    {
        commands.Push( MakeUnique< Command >( args ... ) );
    }
...
And usage of that command buffer would look like:

{% highlight c++ linenos %}
CommandBuffer& cmd = /* get the command buffer from somewhere */;

// ...

cmd.AddCommand< AddComponent< MyComponent > >( entity, my_component );
cmd.AddCommand< RemoveComponent< MyOtherComponent > >( other_entity );

// at some point when no systems are running

cmd.Execute( world );
{% endhighlight %}

Of course, this interface can be simplified greatly by making wrapper functions to add certain commands with less verbosity.
...
Now our `System::Run` needs to be updated to only run a system once all of its dependent systems have run. We might do this
by adding our systems to a job queue

Any systems that don't have a dependency defined can be run in any order, and any of those systems that do not write to memory that the other reads
can be run in parallel. Fortunately, this type of ECS forces us to fully define the function's interface - the data that the function reads from
and writes to - so it is totally possible for us to determine which systems can be run in parallel.

In cases where a system that modifies many components cannot be run in parallel with _any_ other systems, we may still be able to
internally parallelise it. e.g.:

{% highlight c++ linenos %}
void SystemThatCannotBeRunInParallelWithAnythingElse(
    onyx::ecs::Context<
        // maybe add a special context type that implies that this function should not be run in parallel with anything?
    > ctx,
    ...
    #pragma omp parallel for
    for ( int i = 0; i < many_entities.Count(); ++i )
    {
        auto [ id, ... ] = many_entities.nth( i ).Break();
...
Another drawback of this design is the costs of adding or removing components. Both operations are $$O(n)$$, as they must preserve the
...
ECS's work best when we do the exact same thing to a set of entities over and over. But they struggle with reactive behaviour,
we _could_ make queries that check for a _bunch_ of different components, and call callbacks on those components. But that could
ruin our ability to run that system in parallel with others, and is just generally clunky to work with.

It would be better if we could instead send messages between systems. We can already send some information between systems using
our systems' Context arguments, by having one system write to a context argument, and a later system read from the same context argument.

The interface for this messaging system might look something like this:
...
struct MessageHandler
{
    ...
    // add a message of this type for this entity
    template< typename Message >
    void Send( EntityID recipient, Message msg );

    // check for messages of this type for this entity
    template< typename Message >
    Vector< const Message& > Receive( EntityID recipient ) const;

    // check for all messages of this type to any entity
    template< typename Message >
    Vector< Pair< EntityID, const Message& > > ReceiveAll() const;

    // ...
};
{% endhighlight %}

I am not sure of the best way to implement this. We could keep an associative array for each type of message, similar to how
we store an entities components, but allowing for more than one message of a given type per entity. This would also require some
degree of type erasure on the tables themselves, so `UniquePtr`s to them can be stored in the same map.

Using this message handler would then look something like:
...
void GenerateCollisions(
    ...
    auto [ message_handler ] = ctx.Break();

    for ( auto& lhs : colliders )
    {
        auto [ lhs_id, lhs_transform, lhs_collider ] = lhs.Break();

        for ( auto& rhs : colliders )
        {
            auto [ rhs_id, rhs_transform, rhs_collider ] = rhs.Break();

            if ( Collider::AreColliding( lhs_transform, lhs_collider, rhs_transform, rhs_collider ) )
                message_handler.Send( rhs_id, CollidedWithEvent( lhs_id ) );
        }
    }
}

void TakeDamageOnCollision(
    onyx::ecs::Context<
        const MessageHandler
    > ctx,

    const onyx::ecs::Query<
        onyx::ecs::Write< Health >
    >& entities
) {
    auto [ message_handler ] = ctx.Break();

    for ( const auto& [ recipient, collided_with_event ] : message_handler.ReceiveAll< CollidedWithEvent >() )
    {
        if ( auto entity = entities.Get( recipient ) )
        {
            auto [ id, health ] = entity->Break();
...
This pattern would allow us to communicate in a loosely coupled way between systems, without exploding the number
of components we have.

Unfortunately an automatic scheduler would prevent us from reading any messages of any type when any
other system could send a message of any type. So a more specific message handler may be beneficial.
