...
order, and without writing to memory that another system is reading. Ideally we would have a [better solution to scheduling concerns](#scheduling).
...
Now, when we call `AddSystem` a few things will happen:
- `System`'s constructor will infer what context data, and component queries we need
- `QueryManager::Get` will get or create the queries we need
- `Context`'s constructor will assert that the system set has the context the system needs
